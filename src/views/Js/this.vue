...
<CodeEditor code="const sayName = obj.sayName;
sayName(); // undefined或抛出错误（严格模式下 this 为 undefined）
"/>
<p>3. 显式绑定</p>
<p>通过 call、apply 或 bind 方法，可以显式地指定 this。</p>
<CodeEditor code="function greet(greeting){
  console.log(`${greeting}, I am ${this.name}`);
  const person = { name: 'Bob' };
  greet.call(person, 'Hello'); // 输出: Hello, I am Bob
  greet.apply(person, ['Hi']); // 输出: Hi, I am Bob
  const boundGreet = greet.bind(person);
  boundGreet('Hey'); // 输出: Hey, I am Bob
}
"/>
<p>4. 构造函数绑定</p>
<p>通过 new 操作符调用普通函数时，会创建一个新的对象，并将 this 绑定到这个新对象。</p>
<CodeEditor code="function Person(name) {
  this.name = name;
}
...
obj.sayName(); // undefined (this 指向外层作用域的 this，即全局对象)
"/>
<p>2. 箭头函数与普通函数的对比</p>
<CodeEditor code="const obj = {
  name: 'Alice',
...
obj.sayNameArrow(); // undefined (this 指向外层作用域的 this)
"/>
<p>3. 箭头函数在回调中的使用</p>
<p>箭头函数特别适用于嵌套函数或回调函数中，因为它继承了外层作用域的 this，避免了动态绑定问题。</p>
<p>示例：普通函数中的嵌套调用问题</p>
<CodeEditor code="function Timer() {
  this.seconds = 0;
...
const timer = new Timer();
// 输出: 每秒递增的数字，this 绑定到 Timer 实例"/>
<p>4. 箭头函数不适合用作构造函数</p>
<p>箭头函数没有 prototype，因此不能用作构造函数。</p>
<CodeEditor code="const Arrow = () => {};
const obj = new Arrow(); // 抛出错误：Arrow is not a constructor"/>
<h4>call和apply作用一样，区别就是参数区别；apply参数为数据</h4>
<CodeEditor code="getDate.call({year:2022},1,1) // 2022-1-1
getDate.apply({year:2023},[1,1]) // 2023-1-1"/>
